...
export default class ActivitiesApiDev implements IActivityApi {
  activities: IActivity[] = [
    {
      ...
      OPRs: {
        results: [
          { Id: 1, Title: "Robert Porterfield" },
          { Id: 2, Title: "Jeremy Clark" },
        ],
      },
      __metadata: { etag: '"1"' },
    },
  ...
  async fetchActivitiesByNumWeeks(
    numWeeks: number,
    weekStart: Moment,
    userId: number
  ): Promise<any> {
  ...
  async fetchActivitiesByQueryString(query: string): Promise<any> {
    ...
    return this.activities.filter((activity) =>
      query ? activity.ActionTaken.includes(query) : true
    );
  }

  async fetchMarEntriesByDates(
    startDate: Moment,
    endDate: Moment,
    userId: number
  ) {
    await this.sleep();
    return {
      results: this.activities.filter((activity) => activity.IsMarEntry),
    };
  }

  async fetchHistoryEntriesByDates(
    startDate: Moment,
    endDate: Moment,
    userId: number
  ) {
    await this.sleep();
    return {
      results: this.activities.filter((activity) => activity.IsHistoryEntry),
    };
  }
