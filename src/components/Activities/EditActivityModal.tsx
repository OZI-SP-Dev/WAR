...
export const EditActivityModal: FunctionComponent<IEditActivityModalProps> = (props) => {
  ...
  const [selectedDate, setSelectedDate] = useState<Date>(DateUtilities.momentToDate(weekStart));
  const [highlightDates, setHighlightDates] = useState<Date[]>(DateUtilities.getWeek(weekStart));
  ...
  const isOPRInvalid = () => {
    return activity.OPRs && activity.OPRs.results && activity.OPRs.results.length ? false : true;
  }
  ...
  const isReadOnly = () => {
    return !props.canEdit(props.activity) || (DateUtilities.getDate(props.activity.WeekOf) < props.minCreateDate && !props.userIsOrgChief(props.activity.Branch));
  }

  const DatePickerCustomInput = (props: { value?: string | number | string[] }) => (
    <>
  ...
  return (
    <ActivityModal
      ...
      error={props.error}
    >
      <Form id="EditActivityModal" noValidate validated={validated}
        onSubmit={(e) => {
          e.preventDefault();
          e.stopPropagation();
          validateActivity();
        ...
        {props.showHistoryCheck(activity.Branch) &&
          <Form.Group>
            <Form.Check
              ...
              onChange={(e) => updateActivity(e.target.checked, 'IsHistoryEntry')}
              disabled={isReadOnly()}
            />
          </Form.Group>}
