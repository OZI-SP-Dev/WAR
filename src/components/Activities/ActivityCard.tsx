...
export const ActivityCard: React.FunctionComponent<IActivityCardProps> = (
  ...
  return (
    <Card
      className={`activity ${props.className ? props.className : ""}`}
      onClick={(e: any) => {
        if (!showCopyPopover) {
    ...
    >
      <Card.Body>
        ...
        <Row>
          <Col xs={props.review && props.activity.IsMarEntry ? "6" : "12"}>
            <Card.Text as="div">
              ...
              <span style={{ whiteSpace: "pre-line" }}>
                {props.activity.ActionTaken}
              </span>
              <br />
              <strong>Branch: </strong>
              <span>{props.activity.Branch}</span>
              <br />
              <strong>OPRs:</strong>{" "}
              {props.activity.OPRs &&
                props.activity.OPRs.results &&
                props.activity.OPRs.results.map(
                  (OPR: any, index: number, array: any[]) => (
                    <span key={OPR.Id}>
                      {" "}
                      {OPR.Title}
              ...
              {props.activity.IsMarEntry ? (
                <span className="mar">MAR Item</span>
              ) : (
                ""
              )}
              {props.activity.IsHistoryEntry ? (
                <span className="history">History Item</span>
              ) : (
                ""
              )}
            </Card.Text>
          </Col>
          {props.review && props.activity.IsMarEntry && (
            <Col xs="6">
              <Card.Text as="div">
                <strong>MAR Text:</strong>{" "}
                <span style={{ whiteSpace: "pre-line" }}>
                  {props.activity.MARText}
                </span>
              </Card.Text>
            </Col>
          )}
