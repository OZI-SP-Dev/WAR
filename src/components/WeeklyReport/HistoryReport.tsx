...
export const HistoryReport: FunctionComponent<IHistoryReportProps> = ({
  user,
}) => {
  let query = useQuery();
  let defaultQuery = query.getParamOrDefaultString(
    query.params.get("query"),
    "",
    ""
  );
  let defaultOrg = query.getParamOrDefaultString(
    query.params.get("org"),
    RoleUtilities.getReviewDefaultOrg(user),
    ""
  );
  let defaultIncludeSubOrgs = query.getParamOrDefaultBoolean(
    query.params.get("includeSubOrgs"),
    true
  );
  let defaultStartDate = query.getParamOrDefaultDateTime(
    query.params.get("startDate"),
    DateUtilities.getToday().startOf("year")
  );
  let defaultEndDate = query.getParamOrDefaultDateTime(
    query.params.get("endDate"),
    DateUtilities.getToday().endOf("week")
  );
  let defaultOpr = query.getParamOrDefaultString(
    query.params.get("opr"),
  ...
  const submitSearch = async (
    ...
    history.push(
      `/HistoryReport?query=${keyword}&org=${org}&includeSubOrgs=${includeSubOrgs}&startDate=${
        startDate ? startDate.toISOString() : ""
      }&endDate=${endDate ? endDate.toISOString() : ""}&opr=${oprEmail}`
    );
